...
- 启动窗口默认在所在屏幕的工作区居中，按显示缩放调整尺寸，并避开任务栏。“今日 / 课表 / 账户”支持窄窗口与侧栏导航；`Ctrl+1/2/3` 切页，`Ctrl+R` 刷新。
- 右上角刷新与 macOS 保持一致：今日刷新当天，课表刷新所选日期；刷新时原位显示加载指示并禁用重复点击，快速响应也保留至少 450 毫秒反馈。加载状态按账户和日期隔离，未连接时禁用，账户页隐藏课程刷新按钮；`Ctrl+R` 共用刷新逻辑。
...
- 在“账户 → 设置”统一调整系统 / 浅色 / 深色主题、课堂偏好及打开系统通知设置；主题按设备保存，课堂偏好按账户保存。姓名与学号隐藏设置作用于账户列表和课程页。
- 账户保存在 `%LOCALAPPDATA%/UCASSignIn/accounts.dat`，以当前 Windows 用户 DPAPI 加密。缓存与记录按账户隔离。
- 前台自动签到在窗口失去激活、关闭或设备休眠时暂停。计划通知由 Windows 发送，关闭应用不取消有效提醒。
