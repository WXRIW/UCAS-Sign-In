...
struct CourseDetailView: View {
    ...
    private var latestCourse: Course? { model.courses.first { $0.id == course.id && $0.day == course.day } }
    ...
    var body: some View {
                ...
                VStack(spacing: 23) {
                    ...
                    VStack(spacing: 20) {
                        TimelineView(.periodic(from: .now, by: 0.2)) { context in
                            VStack(spacing: 18) {
                                if model.isDemo, let qrImage {
                                    qrGraphic(qrImage)
                                    Label("演示二维码 · 无签到效力", systemImage: "sparkles")
                                        .font(.system(size: 12)).foregroundStyle(Palette.green)
                                } else if let snapshot, snapshot.expiresAt > context.date, scenePhase == .active, let qrImage {
                                    qrGraphic(qrImage)
                                    VStack(spacing: 9) {
                    ...
                    PrimaryButton(title: courseWasRemoved ? "课程已不在最新课表中" : currentCourse.signed ? "已完成签到" : "为本节课程签到", symbol: "checkmark.circle",
                                  loading: model.signingID == course.id) { Task { await model.sign(currentCourse) } }
                        .disabled(!model.canSign(currentCourse))
        ...
        .appSheetSize(width: 520, height: 730)
        .task(id: "\(scenePhase)-\(retry)") { await refreshQR() }
    }
