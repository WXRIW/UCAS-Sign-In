...
final class UCASSignInUITests: XCTestCase {
    ...
    func testRecordsLogoutAndLoginSheet() {
        ...
        tapAfterScrolling(app.buttons["本机签到记录"], in: app)
        ...
        XCTAssertTrue(app.tabBars.buttons["账户"].isSelected)
    ...
    func testCourseQRCodeDemoSignInAndRecord() {
        ...
        XCTAssertTrue(app.navigationBars["课程签到"].waitForExistence(timeout: 5))
    ...
    private func selectTab(_ title: String, in app: XCUIApplication,
        ...
        if tab.exists {
            ...
            topTab.tap()
            let navigationTitle = title == "今日" ? "果壳签到" : title
            XCTAssertTrue(app.navigationBars[navigationTitle].waitForExistence(timeout: 5), file: file, line: line)
        }
    }
