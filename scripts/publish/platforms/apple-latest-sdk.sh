...
# Dedicated latest-SDK packager. Keep this implementation independent from
# apple.sh so experimental toolchain requirements cannot affect stable builds.
...
swift_language_version="${UCAS_LATEST_SWIFT_LANGUAGE_VERSION:-}"
ios_name="UCAS-SignIn-$release_version-ios-latest-sdk-unsigned.ipa"
macos_name="UCAS-SignIn-$release_version-macos-latest-sdk-universal.zip"
macos_pkg_name="UCAS-SignIn-$release_version-macos-latest-sdk-universal.pkg"
...
if [[ ! "$swift_language_version" =~ ^[0-9]+(\.[0-9]+){1,2}$ ]]; then
    printf 'latest SDK 构建缺少或包含无效的 Swift 语言版本：%s\n' \
        "${swift_language_version:-<missing>}" >&2
    exit 2
fi

ios_requirement_build_args=(
    ONLY_ACTIVE_ARCH=NO
    SWIFT_VERSION="$swift_language_version"
)
macos_requirement_build_args=(
    ONLY_ACTIVE_ARCH=NO
    SWIFT_VERSION="$swift_language_version"
)
...
export_macos() {
    ...
    while IFS= read -r -d '' binary; do
        if /usr/bin/file -b "$binary" | grep -q 'Mach-O'; then
            # Xcode 27's lipo accepts one architecture per -verify_arch call.
            /usr/bin/lipo "$binary" -verify_arch arm64
...
run_export '[1/2] 生成 iOS IPA（未签名）' "$ios_name" \
    "$ios_logs_dir/release-ios-latest-sdk.log" export_ios "$ios_artifacts_dir"
run_export '[2/2] 生成 macOS ZIP 和 PKG（Intel + Apple Silicon，App 临时签名）' "$macos_name" \
    "$macos_logs_dir/release-macos-latest-sdk.log" export_macos "$macos_artifacts_dir" "$macos_pkg_name"
