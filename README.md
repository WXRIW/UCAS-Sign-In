...
缓存课表需同步成功后才能签到。网络恢复后可点击「重新同步」，App 回到前台或保持前台时也会自动重试。登录失效时需重新登录。
...
小组件随主 App 同步课程，不会独立联网或执行签到。数据未更新时，打开主 App 刷新课程；演示模式不提供小组件数据。
...
macOS 会在启动时恢复登录，并在打开登录页时填充已记住的账号密码。若系统提示钥匙串授权，输入“登录”钥匙串密码，通常与 Mac 登录密码相同；应用签名变化后可能需要重新授权。
...
在 Xcode 中选择 `UCASSignIn` scheme 和 iPhone / iPad 模拟器，或选择 `UCASSignInMac` scheme 和 **My Mac**，按 `⌘U` 运行 UI 测试。UI 测试以演示模式启动。

## 目录结构

```text
UCASSignIn/
├── Core/                    课程模型、学校接口、时间处理与 Keychain
├── UI/                      SwiftUI 页面与应用状态
...
UCASSignInUITests/           演示模式 UI 测试
UCASSignInMacUITests/        macOS 演示模式 UI 测试
...
- 会话保存在本机 Keychain；只有选择记住密码时才持久保存账号密码。
- 课程缓存按账号和日期隔离，本机保留各账号最近 100 条签到结果，不代表学校完整考勤记录。
- 小组件只共享课程摘要和签到状态，不共享密码或会话令牌。
- App 不申请定位、通讯录或相机权限，未集成广告或统计 SDK。
- 退出真实账号会清除本机凭据、课程缓存、签到记录、待发送提醒及小组件课程数据。
