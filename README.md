...
- **版本更新**：非商店版每日从 GitHub 检查一次最新正式 Release，也可在设置中手动检查；Windows 商店版由 Microsoft Store 检查、下载和安装更新。
...
iPhone / iPad / Android 下拉刷新课程；Mac 点击刷新按钮或按 `⌘R`，Windows 点击刷新按钮或按 `Ctrl+R`。课表页刷新所选日期，切换页面不会重置日期。
...
Apple 客户端升级后，原有单账户登录、课程缓存、签到记录和课堂偏好会自动保留。演示模式使用示例数据，不添加或覆盖真实账户。

### 自动签到与课程提醒

在「账户 → 课堂偏好」设置账户默认的自动签到、手动签到二次确认和课程提醒。课程提醒需要允许通知权限，默认在开课前 10 分钟发送，也可选择 5、15 或 30 分钟。

「课程」页列出当前学期的全部课程。每门课程都可选择跟随账户默认值、单独启用或停用自动签到、二次确认和提醒，并可覆盖提醒时间。**只有当前账户运行自动签到和课程提醒**；切换会清除原账户的待发送提醒，并按目标账户的偏好和已同步课程重新安排。

平台运行方式有所不同：iOS / iPadOS 受系统后台限制，自动签到仅在 App 前台活跃时运行；macOS 在 App 仍运行时可关闭或最小化窗口，Windows 可最小化窗口或切换到其他应用；Android 使用带常驻通知的前台服务在后台运行，系统省电、自启动限制或强行停止 App 仍可能造成延迟或中断。设备睡眠或退出桌面客户端后，自动签到会暂停。课程提醒不会触发签到。
...
Apple 客户端使用 **Swift + SwiftUI**，工程位于 `apps/apple/`。使用 Xcode 打开 `apps/apple/UCASSignIn.xcodeproj`。

完整环境要求、签名配置、构建、测试和发布方法见 [Apple 开发说明](apps/apple/README.md)。
...
- 账户列表、各账户的会话和密码保存在本机：Apple 使用 Keychain，Windows 使用当前用户 DPAPI，Android 使用 Keystore/AES-GCM。只有选择记住密码时才持久保存密码。本功能不提供跨设备账户同步。
