...
Apple、Android 和 Windows 的日视图与周视图均共用本机的整学期课表缓存。App 运行期间，距上次完整同步成功满七天后自动更新；每次冷启动后首次打开某个日期时检查一次，课程增删、时间、教室或教师发生变化时，重新同步所有已缓存周。仅签到状态变化时单独更新状态。仅周视图底部显示所查看学期最近一次完整更新的日期和时间；未变化的单日检查或失败的同步不会修改这个时间。

缓存课表需同步成功后才能签到；网络恢复后可点击「重新同步」。
